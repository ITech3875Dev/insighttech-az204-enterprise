# Intermediate Lab 03 - Break-Glass and Privileged Access Operations
...
## Objective
Simulate a full break-glass workflow safely:
...
Create runbook-break-glass.md with these sections:
- Preconditions
- Trigger criteria
...
az role assignment list --scope "$TARGET_SCOPE" -o json > evidence-breakglass-baseline.json
```

### PowerShell
```powershell
Select-AzSubscription -SubscriptionId $SubscriptionId
Get-AzRoleAssignment -Scope $TargetScope | ConvertTo-Json -Depth 8 | Out-File evidence-breakglass-baseline-pwsh.json
```
...
### Azure CLI
```bash
az ad group create --display-name "$BREAK_GLASS_GROUP" --mail-nickname "$BREAK_GLASS_GROUP"
BREAK_GLASS_GROUP_ID=$(az ad group show --group "$BREAK_GLASS_GROUP" --query id -o tsv)
echo "$BREAK_GLASS_GROUP_ID" > evidence-breakglass-group-id.txt
```
...
$bg.Id | Out-File evidence-breakglass-group-id-pwsh.txt
```
...
az role assignment create \
  --assignee-object-id "$BREAK_GLASS_GROUP_ID" \
